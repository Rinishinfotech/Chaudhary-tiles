    ...
    <Layout searchPlaceholder="Search report metrics...">
      ...
      <div className="section-grid">
        ...
        <div className="card-box">
          ...
          <table><thead><tr><th>Category</th><th>Recipient</th><th>Logged By</th><th>Amount</th></tr></thead><tbody>
            {data.recentExpenses.map((e, i) => <tr key={i}><td><strong>{e.category}</strong></td><td>{e.plant}</td><td>{e.employee}</td><td><strong style={{ color: '#dc3545' }}>{inr(e.amount)}</strong></td></tr>)}
            {data.recentExpenses.length === 0 && <tr><td colSpan="4" style={{ textAlign: 'center', color: '#888' }}>No expenses logged</td></tr>}
          </tbody></table>
        </div>
      </div>
    </Layout>
